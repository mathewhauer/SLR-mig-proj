...
files <- paste0("R/DATA-RAW/Dat/", list.files(path = "./R/DATA-RAW/Dat/",pattern = 'rcp26\\.csv'))
files <- files[files != "R/DATA-RAW/Dat/BlockgroupProjectedEAE.v3.LA.rcp26.csv" ]
files2 <- read_csv("./R/DATA-RAW/Dat/BlockgroupProjectedEAE.v3.LA.rcp26.csv") %>%
...
together3 <- rbind(together2, df) %>%
  ...
  mutate(Inundated = case_when(
    ...
    TRUE ~ Inundated),
    EAE = case_when(
      is.na(`Exp. Ann. Flood`) ~ (lag(`Exp. Ann. Flood`) + lead(`Exp. Ann. Flood`))/2,
      TRUE ~ `Exp. Ann. Flood`)
  ) 
together3in <- together3 %>%
  filter(#prob2 == "p95", ###### USING the 95th percentile of RCP 2.6. See https://agupubs.onlinelibrary.wiley.com/doi/full/10.1002/2014EF000239 TABLE 1 for the SLR amount in 2100
         SSP2 == "SSP2",
         year >=2020) %>%
  dplyr::select(-`Exp. Ann. Flood`, -`100-year FP`, -EAE)
together3eae <- together3 %>%
  filter(prob2 == "p50", ###### USING the 95th percentile of RCP 2.6. See https://agupubs.onlinelibrary.wiley.com/doi/full/10.1002/2014EF000239 TABLE 1 for the SLR amount in 2100
         SSP2 == "SSP2",
         year >=2020) %>%
  ungroup() %>%
  dplyr::select(-`Exp. Ann. Flood`, -`100-year FP`, -Inundated, -prob2)

together3 <- left_join(together3in, together3eae)

## Loading in the model coefficients.
coeffs <- read_csv("./R/DATA-PROCESSED/modelcoeffs.csv")

z <- crossing(together3, coeffs) %>%
  mutate(#reduce_in = exp(b * log(1-Inundated)),
    reduce_in = exp(b * log(1-Inundated) + c * log(1-Inundated)^2),
         reduce_eae = exp(b * log(1-`EAE`)))

write_csv(z, "./R/DATA-PROCESSED/atriskpops26.csv")
write_csv(together3, "./R/DATA-PROCESSED/basepercentagepoploss26.csv")
